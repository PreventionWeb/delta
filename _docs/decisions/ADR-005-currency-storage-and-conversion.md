...
usd_conversion_rate: numeric("usd_conversion_rate", {
	precision: 19,
	scale: 6,
});
...
export const exchangeRatesTable = pgTable(
	...
	(table) => ({
		pk: primaryKey({
			columns: [table.fromCurrency, table.toCurrency, table.effectiveDate],
		}),
	}),
);
```
...
**Current rate projection (analytics mode)**: Aggregate dashboard views may optionally re-convert historical base amounts using today's rate for trend analysis. This mode is explicitly labelled as _"indicative — not accounting accurate"_ in both the UI and API responses. The `_usd` columns are never changed — only the display computation changes.
