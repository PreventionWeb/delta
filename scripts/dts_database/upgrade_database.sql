\encoding UTF8
SHOW client_encoding;

\echo Checking current database version...

-- ================================
-- Upgrade 0.1.1 -> 0.1.3
-- Note: the app version was named here 0.1.2 but the version in db was saved as 0.1.1
-- ================================
SELECT (version_no='0.1.1') AS is_version_0_1_1 FROM dts_system_info LIMIT 1 \gset
\if :is_version_0_1_1
    \echo Upgrading from 0.1.1 to 0.1.3...
    \ir upgrade_from_0.1.2_to_0.1.3.sql
\endif

-- ================================
-- Upgrade 0.1.3 -> 0.2.0
-- ================================
SELECT (version_no='0.1.3') AS is_version_0_1_3 FROM dts_system_info LIMIT 1 \gset
\if :is_version_0_1_3
    \echo Upgrading from 0.1.3 to 0.2.0...
    \ir upgrade_from_0.1.3_to_0.2.0.sql
\endif


-- ================================
-- Upgrade 0.2.0 -> 0.2.1
-- ================================
SELECT (version_no='0.2.0') AS is_version_0_2_0 FROM dts_system_info LIMIT 1 \gset
\if :is_version_0_2_0
    \echo Upgrading from 0.2.0 to 0.2.1...
    \ir upgrade_from_0.2.0_to_0.2.1.sql
\endif

-- ================================
-- Upgrade 0.2.1 -> 0.2.2
-- ================================
SELECT (version_no='0.2.1') AS is_version_0_2_1 FROM dts_system_info LIMIT 1 \gset
\if :is_version_0_2_1
    \echo Upgrading from 0.2.1 to 0.2.2...
    \ir upgrade_from_0.2.1_to_0.2.2.sql
\endif

-- ================================
-- Upgrade 0.2.2 -> 0.2.3
-- ================================
SELECT (version_no='0.2.2') AS is_version_0_2_2 FROM dts_system_info LIMIT 1 \gset
\if :is_version_0_2_2
    \echo Upgrading from 0.2.2 to 0.2.3...
    \ir upgrade_from_0.2.2_to_0.2.3.sql
\endif

-- ================================
-- Upgrade 0.2.3 -> 0.3.0
-- ================================
SELECT (version_no='0.2.3') AS is_version_0_2_3 FROM dts_system_info LIMIT 1 \gset
\if :is_version_0_2_3
    \echo Upgrading from 0.2.3 to 0.3.0...
    \ir upgrade_from_0.2.3_to_0.3.0.sql
\endif