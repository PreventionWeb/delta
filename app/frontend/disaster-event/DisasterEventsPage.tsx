...
export default function DisasterEventsPage({
	...
	const [recordingOrganizationFilter, setRecordingOrganizationFilter] =
		useState(filters?.recordingOrganization ?? "");
	...
	const [specificHazardFilter, setSpecificHazardFilter] = useState<
		string | null
	...
	const statusOptions = [
		{
			label: ctx.t({ code: "approval_status.draft", msg: "Draft" }),
			value: "draft",
		},
		{
			label: ctx.t({
		...
		{
			label: ctx.t({ code: "approval_status.validated", msg: "Validated" }),
			value: "validated",
		},
		{
			label: ctx.t({ code: "approval_status.published", msg: "Published" }),
			value: "published",
		},
	].filter((status) =>
		canEdit
	...
	const actionsBodyTemplate = (row: (typeof data)[number]) => {
		const canShowEdit = canEdit && !isEditRestrictedStatus(row.approvalStatus);
		const canShowDelete =
		...
		return (
			<div className="flex w-full items-center justify-end gap-1">
				<Link to={`/${ctx.lang}/disaster-event/${row.id}`}>
					<Button type="button" text aria-label="View">
						<i className="pi pi-eye" aria-hidden="true" />
					</Button>
				</Link>
				{canShowEdit ? (
					<Link to={`/${ctx.lang}/disaster-event/edit/${row.id}`}>
						<Button type="button" text aria-label="Edit">
				...
				{canShowDelete ? (
					<Link to={`/${ctx.lang}/disaster-event/delete/${row.id}`}>
						<Button type="button" text severity="danger" aria-label="Delete">
	...
	const statusBodyTemplate = (row: (typeof data)[number]) => {
		return (
			<span
				...
				title={String(row.approvalStatus ?? "") || undefined}
			>
				<span
			...
			<div>
				<div className="mb-6 flex flex-wrap items-center justify-between gap-3">
					<div>
						...
						<p className="text-[#334155]">
							{ctx.t({
								code: "disaster_event_data_management",
								msg: "Disaster event data management",
							})}
						</p>
					</div>
				...
				>
						...
						<div className="w-full sm:min-w-[22rem] sm:flex-[1.6]">
							...
							<div className="relative">
								<InputText
									...
									type="text"
									placeholder={ctx.t({
										code: "search_by_event_name",
										msg: "Search by event name...",
									})}
					...
					{isFiltersExpanded && (
						<>
								...
								<div className="w-full sm:min-w-[14rem] sm:flex-1">
									...
									<Dropdown
										...
										onChange={(e) => {
											...
											if (nextValue.trim()) {
												nextSearchParams.set("hazardCluster", nextValue);
											} else {
												nextSearchParams.delete("hazardCluster");
											}
											nextSearchParams.delete("specificHazard");
								...
								<div className="w-full sm:min-w-[14rem] sm:flex-1">
									...
									<Dropdown
										...
										onChange={(e) => {
											...
											if (nextValue.trim()) {
												nextSearchParams.set("specificHazard", nextValue);
											} else {
												nextSearchParams.delete("specificHazard");
											}
											nextSearchParams.delete("page");
							...
							<div
								className="mt-4 mb-6 border-t border-slate-200"
								aria-hidden="true"
							/>
						</>
					)}
							...
							<div className="flex items-center gap-2">
								<Checkbox
									...
									onChange={(e) => {
										...
										updateFilterParam("viewMyRecords", isChecked ? "true" : "");
									}}
								/>
								<label
				...
				>
					...
					<span className="inline-flex items-center gap-2">
						...
						<span>
							{ctx.t({ code: "approval_status.draft", msg: "Draft" })}
						</span>
					</span>
					<span className="inline-flex items-center gap-2">
						<span
							className={getStatusCircleClassName("waiting-for-validation")}
						/>
						<span>
							{ctx.t({
								code: "approval_status.waiting_for_validation",
					...
					<span className="inline-flex items-center gap-2">
						...
						<span>
							{ctx.t({ code: "approval_status.validated", msg: "Validated" })}
						</span>
					</span>
					<span className="inline-flex items-center gap-2">
						<span className={getStatusCircleClassName("published")} />
						<span>
							{ctx.t({ code: "approval_status.published", msg: "Published" })}
						</span>
					</span>
				</div>
				...
				<div className="w-full overflow-x-auto rounded-lg border border-slate-200 [&_.p-datatable]:w-full [&_.p-datatable-wrapper]:w-full [&_.p-datatable-table]:w-full [&_.p-datatable-table]:min-w-[72rem] [&_.p-datatable-table]:table-auto">
					<DataTable
						...
						dataKey="id"
						emptyMessage={ctx.t({
							code: "no_disaster_events_found",
							msg: "No disaster events found",
						})}
						sortMode="multiple"
						removableSort
						size="small"
					...
					>
						...
						<Column
							...
							headerClassName="min-w-[6rem] bg-gray-100 px-2 py-3 border-b border-gray-200"
							bodyClassName="min-w-[6rem] px-2 py-3 border-b border-gray-200"
						/>
					</DataTable>
				</div>
