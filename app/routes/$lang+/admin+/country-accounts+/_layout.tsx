...
export function getCountryAccountTypeLabel(ctx: DContext, type: string) {
	switch (type) {
		case "Official":
			return ctx.t({
...
export default function CountryAccountsLayout() {
	...
	function countryTypeBodyTemplate(
		...
		return countryType === COUNTRY_TYPE.FICTIONAL ? (
			<Tag value={countryType} severity="warning" />
		) : (
			<Tag value={countryType} />
		);
	}
