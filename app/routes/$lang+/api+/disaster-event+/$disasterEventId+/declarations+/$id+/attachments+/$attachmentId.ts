...
import {
	...
	getDeclarationAttachmentById,
	isMultipartRequest,
	updateDeclarationAttachment,
...
function getParams(params: LoaderFunctionArgs["params"]) {
	...
	return { disasterEventId, declarationId, attachmentId };
}

export const loader = authLoaderApi(async (args: LoaderFunctionArgs) => {
	const { disasterEventId, declarationId, attachmentId } = getParams(
		args.params,
	);
	...
	const record = await getDeclarationAttachmentById({
		countryAccountsId,
		disasterEventId,
		declarationId,
		attachmentId,
...
export const action = async (args: ActionFunctionArgs) => {
	...
	if (args.request.method === "PUT") {
		...
		if (isMultipartRequest(args.request)) {
			...
			payload = await uploadDeclarationAttachmentFile({
				countryAccountsId,
				disasterEventId,
				declarationId,
				uploadedFile,
		...
		const updated = await updateDeclarationAttachment({
			countryAccountsId,
			disasterEventId,
			declarationId,
			attachmentId,
	...
	if (args.request.method === "DELETE") {
		await deleteDeclarationAttachment({
			countryAccountsId,
			disasterEventId,
			declarationId,
			attachmentId,
