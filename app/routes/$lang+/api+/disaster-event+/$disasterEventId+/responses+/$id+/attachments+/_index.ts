...
function getParams(params: LoaderFunctionArgs["params"]) {
	...
	return { disasterEventId, responseId };
}

export const loader = authLoaderApi(async (args: LoaderFunctionArgs) => {
	const { disasterEventId, responseId } = getParams(args.params);
	...
	const data = await listResponseAttachments({
		countryAccountsId,
		disasterEventId,
		responseId,
	});
...
export const action = async (args: ActionFunctionArgs) => {
	...
	if (isMultipartRequest(args.request)) {
		...
		const created = [] as Array<Awaited<ReturnType<typeof createResponseAttachment>>>;
		for (const uploadedFile of uploadedFiles) {
			const payload = await uploadResponseAttachmentFile({
				countryAccountsId,
				disasterEventId,
				responseId,
				uploadedFile,
			...
			const record = await createResponseAttachment({
				countryAccountsId,
				disasterEventId,
				responseId,
				payload,
	...
	const created = await createResponseAttachment({
		countryAccountsId,
		disasterEventId,
		responseId,
		payload,
