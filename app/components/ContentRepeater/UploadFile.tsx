...
class ContentRepeaterUploadFile {
	...
	static save(
		...
		const tenantId: string | null =
			countryAccountsId || this.extractTenantIdFromItems(items) || null;
		...
		const updatedItems = items.map((item) => {
			...
			if (item.file.tenantPath) {
				...
				tempCandidates.push(
					path.resolve(publicPath, cleanTenant, cleanTempPath, originalName),
				);
			}
			...
			// 4. No tenant (legacy)
			tempCandidates.push(
				path.resolve(publicPath, cleanTempPath, originalName),
			);
			...
			if (tenantId) {
				...
				item.file.tenantPath = path.join(
					BASE_UPLOAD_PATH,
					`tenant-${tenantId}`,
				);
			} else {
