...
type GeometrySchema = z.ZodDiscriminatedUnion<
	"type",
	[
...
export function validateGeoJSON(data: unknown): {
	...
	try {
		...
		return {
			valid: false,
			error: "GeoJSON validation error: " + (e as Error).message,
		};
	}
}
 ...
 * @returns Validation result with success flag and error message if failed
 */
function validateCoordinateRanges(geoJson: any): {
	valid: boolean;
	error?: string;
} {
	try {
		...
		const validateCoord = (coord: number[], path: string): string | null => {
			if (coord.length < 2)
				return `${path}: Coordinate must have at least 2 values`;
			const [lon, lat] = coord;

			if (lon < -180 || lon > 180)
				return `${path}: Longitude ${lon} out of range (-180 to 180)`;
			if (lat < -90 || lat > 90)
				return `${path}: Latitude ${lat} out of range (-90 to 90)`;
		...
		return {
			valid: false,
			error: "Coordinate validation error: " + (e as Error).message,
		};
	}
}
...
export function validateCSVStructure(
	...
	try {
		...
		const geodataIndex = headers.findIndex(
			(h) => h.toLowerCase() === "geodata",
		);

		// Validate language columns (all columns after the required ones)
		const languageColumns = headers.filter(
			(_, i) => ![idIndex, parentIndex, geodataIndex].includes(i),
		);

		const invalidLangs = languageColumns.filter(
		...
		if (languageColumns.length === 0) {
			return {
				valid: false,
				error: "At least one language column is required",
			};
		}
		...
		for (let i = 0; i < rows.length; i++) {
			...
			if (!geodata) {
				return {
					valid: false,
					error: `Row ${i + 1} (id: ${id}) has an empty 'geodata' field`,
				};
			}
			if (!geodata.toLowerCase().endsWith(".geojson")) {
				return {
					valid: false,
					error: `Row ${i + 1} (id: ${id}) has invalid geodata filename. Must end with .geojson`,
				};
			}
		...
		return {
			valid: false,
			error: "CSV validation error: " + (e as Error).message,
		};
	}
}
...
export function detectCircularReferences(
	divisions: Record<
		string,
		{ parent: string; name: Record<string, string>; geodata: string }
	>,
): { valid: boolean; error?: string } {
...
export function validateParentChildRelationships(
	...
	if (missingParents.length > 0) {
		...
		const additional =
			missingParents.length > 3 ? ` and ${missingParents.length - 3} more` : "";
...
export function extractDivisionHierarchy(
	...
	console.log(
		`Total features processed: ${features.length}, Valid divisions: ${divisions.length}`,
	);
