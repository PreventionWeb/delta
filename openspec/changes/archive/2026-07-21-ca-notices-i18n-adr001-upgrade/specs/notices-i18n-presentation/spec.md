...
its non-Response fallback message via `useTranslation("notices")`'s `t("error.generic_retry")`
...
- **THEN** it MUST call `t(...)` from `useTranslation("notices")` rather than rendering a
...
content SHALL contain only the `view` key migrated from the old system's `common.view` code.
This change SHALL NOT modify `ActionLinks.tsx`, `View.tsx`, `action_links.tsx`, `view_form.tsx`,
