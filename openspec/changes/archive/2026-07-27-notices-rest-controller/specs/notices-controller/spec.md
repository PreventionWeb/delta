...
> **Revised (ADR-008, design.md Decision 18)**: Notices content is single-locale. `title`/`body`
> are plain strings, not `LocaleMap`s — there is no per-request locale resolution for content.
> Requirements below reflect this; scenarios that tested the old locale-map fallback behavior
> are removed, not reworded, since that behavior no longer exists.
